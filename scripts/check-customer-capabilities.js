...
const forbidden=/restaurant|comanda|balc[aã]o|gaveta|pizzaria|autoatendimento|terminal\s+pdv|self-service/i;
...
const ids=new Set((manifest.capabilities||[]).filter(x=>x.status==='available').map(x=>x.id));for(const id of ['master-data','inventory','procurement','sales-admin','fiscal-interoperability','finance','reports','backup'])if(!ids.has(id))throw new Error(`Required released capability missing: ${id}`);
