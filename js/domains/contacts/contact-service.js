...
function createContactService({db,now=()=>new Date().toISOString(),idFactory=p=>`${p}-${randomUUID()}`}={}){
 ...
 function updateContact(id,input={},actor=null){assertRole(actor,['admin','manager']);const current=getContact(id);if(!current)throw new Error('Contato nao encontrado.');const name=input.name===undefined?current.name:String(input.name||'').trim();if(!name)throw new Error('Nome do contato obrigatorio.');const taxId=input.taxId===undefined?current.taxId:(String(input.taxId||'').trim()||null);const email=input.email===undefined?current.email:(String(input.email||'').trim()||null);const phone=input.phone===undefined?current.phone:(String(input.phone||'').trim()||null);const ts=String(now());db.prepare('UPDATE contacts SET name=?,tax_id=?,email=?,phone=?,updated_at=? WHERE id=?').run(name,taxId,email,phone,ts,current.id);writeAudit(db,{action:'contacts.update',entity:'contact',entityId:current.id,actor,context:{before:{name:current.name,taxId:current.taxId,email:current.email,phone:current.phone},after:{name,taxId,email,phone}}},now);return getContact(current.id);}
 ...
 function list({kind=null,includeInactive=false,query=null}={}){const clauses=[],params=[];if(kind){clauses.push('(kind=? OR kind=\'BOTH\')');params.push(String(kind).toUpperCase());}if(!includeInactive)clauses.push('active=1');if(query){clauses.push('(LOWER(name) LIKE ? OR LOWER(COALESCE(tax_id,\'\')) LIKE ? OR LOWER(COALESCE(email,\'\')) LIKE ?)');const q=`%${String(query).toLowerCase()}%`;params.push(q,q,q);}return db.prepare(`SELECT * FROM contacts${clauses.length?` WHERE ${clauses.join(' AND ')}`:''} ORDER BY name,id`).all(...params).map(map);}
 ...
 return{createCustomer,createSupplier,getContact,updateContact,requireCustomer,requireSupplier,list,setActive};
}
