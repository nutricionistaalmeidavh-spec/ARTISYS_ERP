...
function createLocalServer({runtime,host='127.0.0.1',port=4174,bodyLimitBytes=1024*1024}={}){
 if(!runtime)throw new TypeError('runtime is required.');const sessions=createSessionService();
 const routers=[createAuthRouter({runtime,sessions,bodyLimitBytes}),createAdminRouter({runtime,sessions,bodyLimitBytes}),createMasterDataRouter({runtime,sessions,bodyLimitBytes}),createInventoryRouter({runtime,sessions,bodyLimitBytes}),createProcurementRouter({runtime,sessions,bodyLimitBytes}),createSalesAdminRouter({runtime,sessions,bodyLimitBytes}),createFinanceRouter({runtime,sessions,bodyLimitBytes}),createReportingRouter({runtime,sessions,bodyLimitBytes})];
 let server=null;
 ...
 return{start,stop,sessions,get running(){return Boolean(server);}};
}
