...
function createOperationsRouter({runtime,sessions,bodyLimitBytes=1024*1024}={}){
 return async(req,res,url)=>{
  ...
  try{
   ...
   if(p==='/api/v1/ops/service-orders'){
    if(req.method==='GET'){
     const filtered=['status','customerId','technicianUserId','limit','offset'].some(k=>url.searchParams.has(k));
     if(filtered)json(res,200,runtime.serviceOrders.list({status:url.searchParams.get('status'),customerId:url.searchParams.get('customerId'),technicianUserId:url.searchParams.get('technicianUserId'),limit:url.searchParams.get('limit'),offset:url.searchParams.get('offset')},a));
     else json(res,200,runtime.operations.listServiceOrders(a));
     return true;
    }
    if(req.method==='POST'){json(res,201,runtime.operations.createServiceOrder(await d(),a));return true;}
   }
   ...
   if((m=pathMatch(p,'/api/v1/ops/service-orders/:id/complete'))&&req.method==='POST'){json(res,200,runtime.serviceOrders.complete(m.id,await d(),a));return true;}
   if((m=pathMatch(p,'/api/v1/ops/service-orders/:id/cancel'))&&req.method==='POST'){json(res,200,runtime.serviceOrders.cancel(m.id,await d(),a));return true;}
   if((m=pathMatch(p,'/api/v1/ops/service-orders/:id'))){if(req.method==='GET'){const result=runtime.serviceOrders.get(m.id,a);if(!result)throw Object.assign(new Error('OS nao encontrada.'),{statusCode:404});json(res,200,result);return true;}if(req.method==='PATCH'){json(res,200,runtime.serviceOrders.updateOpen(m.id,await d(),a));return true;}}
