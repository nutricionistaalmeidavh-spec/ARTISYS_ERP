...
function assertEventType(value) {
  if (typeof value !== 'string' || value.trim() === '') {
    throw new TypeError('Domain event type must be a non-empty string');
  }
}
...
class DomainEventBus {
  ...
  publish(event) {
    ...
    const handlers = this._handlersFor(event.type), failures = [];
    let delivered = 0;
    for (const handler of handlers) {
      try {
        const result = handler(event);
        if (result && typeof result.then === 'function') throw new TypeError('DomainEventBus subscribers must be synchronous when using publish()');
        delivered += 1;
      } catch (error) {
  ...
  clear(eventName) {
    if (eventName === undefined) { this._subscribers.clear(); return; }
    assertEventType(eventName); this._subscribers.delete(eventName);
  }
