...
let runtime=null;let localServer=null;let baseUrl=null;let mainWindow=null;let updater=null;let fiscalSidecar=null;let fiscalCredentialStore=null;
async function boot(){
  ...
  registerImportBridge({ipcMain,app});registerDocumentBridge({ipcMain,app});registerAttachmentBridge({ipcMain,app});registerFiscalCredentialIpc({ipcMain,dialog,credentialStore:fiscalCredentialStore,getParentWindow:()=>mainWindow});
