...
function publicConnection(input) {
  ...
  try {
    const connection = validateConnection(input);
    const configured = connection.provider === 'focus' ? Boolean(String(input.token || '').trim()) : true;
...
module.exports={PROVIDERS,ENVIRONMENTS,DOCUMENT_TYPES,validateConnection,validateSecretConnection,publicConnection,validateReference};
